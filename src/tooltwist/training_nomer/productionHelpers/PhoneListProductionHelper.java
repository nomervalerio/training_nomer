...
public class PhoneListProductionHelper extends WbdProductionHelper {

	public PhoneListProductionHelper(Properties params) {
		super(params);
	...
	public XData preFetch(UimData ud) throws Exception {
			...
			whereClause.append("<expr op='and' returnType='char'>");
				...
				whereClause.append("<expr op='or' returnType='char'>");
					whereClause.append("<expr op='like' returnType='char' >");
						whereClause.append("<operand>phoneId</operand>");
						whereClause.append("<operand type='literal'>%"+filter+"%</operand>");
					whereClause.append("</expr>");
					whereClause.append("<expr op='like' returnType='char' >");
						whereClause.append("<operand>phoneType</operand>");
						whereClause.append("<operand type='literal'>%"+filter+"%</operand>");
					whereClause.append("</expr>");
					whereClause.append("<expr op='like' returnType='char' >");
						whereClause.append("<operand>completeNumber</operand>");
						whereClause.append("<operand type='literal'>%"+filter+"%</operand>");
					whereClause.append("</expr>");
				whereClause.append("</expr>");
			whereClause.append("</expr>");
		...
		Xpc xpc2 = ud.getXpc();
		xpc2.start("phinza.D.personPhone", "select");
